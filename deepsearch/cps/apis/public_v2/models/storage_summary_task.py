...
class StorageSummaryTask(BaseModel):
    """
    StorageSummaryTask
    """ # noqa: E501
    ...
    proj_key: StrictStr
    task_id: StrictStr
    __properties: ClassVar[List[str]] = ["dc_key", "kg_key", "kind", "proj_key", "task_id"]

    @field_validator('kind')
    def kind_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['project_task', 'celery_task']):
            raise ValueError("must be one of enum values ('project_task', 'celery_task')")
        return value
